...
router.put('/profile/:user_id', (request, response) => {
  console.log("putmethode called");
  const { user_id } = request.params
  const {first_Name,last_Name, email_id,address,phone_no,password} = request.body
  //const encryptedPassword = String(cryptoJs.SHA256(password))
  console.log(""+first_Name,last_Name, email_id,address,phone_no,password,user_id);
  db.query(
    `UPDATE users
    SET
      first_Name = ?,last_Name = ?,email_id = ?,address = ?,phone_no = ? ,password = ?
    WHERE
      user_id = ?;
    `,
    [first_Name,last_Name,email_id,address,phone_no,password,user_id],
    (error, result) => {
      response.send(utils.createResult(error, result))
    }
  )
